...
void setup(){
  ...
  while (true){
    ...
    repeat:
      ...
      if (timer.seeTime() > 100 && !dont_enter_here_again){
        OUT(Serial.println("Taking a while..."));
        dont_enter_here_again = true;
      }
    ...
    if (total / count < 0){
      dual_motor_driver.motora.setVelocity(0);
      for(int i = 10; i > 0; --i){
        dual_motor_driver.motorb.setVelocity(wheel_speed/i);
        delay(1000);
      }

    } else{
      dual_motor_driver.motorb.setVelocity(0);
      for(int i = 10; i > 0; --i){
        dual_motor_driver.motora.setVelocity(wheel_speed/i);
        delay(1000);
      }
    }
